        ...
        if (response != null) {
            ...
            if (id.isNotBlank() || name.isNotBlank()) {
                return User(
                    ...
                    image = if (userObj.isNull("image")) null else userObj.optString("image", null),
                    email = userObj.optString("email", ""),
                    createdAt = userObj.optString("created_at", userObj.optString("createdAt", "")),
                    password = userObj.optString("password", ""),
                    updatedAt = userObj.optString("updated_at", userObj.optString("updatedAt", "")),
                    description = userObj.optString("description", ""),
                    followerCount = userObj.optLong("followerCount", userObj.optLong("followersCount", 0)),
                    followingCount = userObj.optLong("followingCount", userObj.optLong("followingsCount", 0)),
                    followers = parsefollow(userObj.optJSONArray("followers")),
                    followings = parsefollow(userObj.optJSONArray("followings")),
                )
            }
